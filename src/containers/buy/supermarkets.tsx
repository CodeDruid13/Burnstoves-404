...
const SupermarketsSection = ({ shopdata }: { shopdata?: any }) => {
  var aquaticCreatures =
    shopdata?.buyLocations?.data[0]?.attributes?.shops.filter(function (
      creature: any,
    ) {
      return creature?.Type == "Supermarkets";
    });

  return (
    <Box width="full" py={14} bg={"white"}>
        ...
        >
          {aquaticCreatures?.map((item: any) => (
            <VStack
              ...
              justify={"center"}
            >
              <Text textAlign={"center"}>{item.purchase_title}</Text>
            </VStack>
          ))}
        </SimpleGrid>
      </Container>
...
export default SupermarketsSection;
